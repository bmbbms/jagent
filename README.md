...
- 运行时已拆分为 `planner / bridge / executor` 阶段
- 已持久化 observation / tool call / data access
- 已打通 `runtime_session_id` 到：
  - `tool_calls`
  - `data_access_logs`
...
| `ACQUIRING_AI_MCP_CONFIG_PATH` | `config/mcp.example.json` | MCP 配置路径 |
...
| `ACQUIRING_AI_INTERNAL_TOOL_PROVIDER_BACKEND` | `local_db` | 内部工具 Provider |
| `ACQUIRING_AI_AGENTSCOPE_USE_MODEL_BRIDGE` | `false` | 是否启用 AgentScope 模型桥接 |
